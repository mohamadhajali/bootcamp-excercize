...
let emailusers = users.map((u) => {
  return { name: u.email, company: u.company.name };
});
...
*/
checkZipCode = function (users) {
  if (users.address.zipcode.charAt(0) == 5) {
    return users.address.zipcode;
  }
};
let zipCodeUsers = users.filter((u) => checkZipCode(u));
...
*/
checkZipCode2 = function (users) {
  if (users.address.zipcode.charAt(0) == 5) {
    return users.address.zipcode;
  }
};
let zipCodeUsers2 = users.filter((u) => checkZipCode2(u)).map((u) => u.id);
console.log("Exercise 3");

console.log(zipCodeUsers2);
/*
Exercise 5
